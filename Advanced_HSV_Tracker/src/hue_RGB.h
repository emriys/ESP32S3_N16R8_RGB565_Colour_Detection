...
TrackingResult findColorBlob(camera_fb_t *fb) {
  ...
  for (int i = 0; i < total_pixels; i++) {
    ...
    // 2. Swap the high and low bytes to correct any hardware endianness mismatch
    p = __builtin_bswap16(p); 
    
    float h, s, v;
    rgb565_to_hsv(p, h, s, v);

    // 3. Check if Hue matches (with wrap-around support)
