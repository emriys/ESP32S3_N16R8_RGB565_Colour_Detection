...
TrackingResult findColorBlob(camera_fb_t *fb) {
  ...
  // The ultimate speed loop: Scan every pixel in the frame
  for (int i = 0; i < total_pixels; i++) {
    ...
    if (r >= min_r && r <= max_r &&
      ...
      // Add to our Center of Mass calculation
      sum_x += current_x;
      sum_y += current_y;
      result.pixelCount++;
    }
  ...
  // 4. Final Math: Average the coordinates to find the center
...
void initVision() {
  ...
  // Configure the robot to search for your Blue Hex Code.
  // The '3' is our tolerance. It means look for +/- 3 shades of Red and Blue, 
  // and +/- 6 shades of Green around our perfect target.
